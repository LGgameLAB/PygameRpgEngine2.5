...
class player:
    ...
    def __init__(self, startX, startY, startDir, animations, moveBuffer, speed, animationSpeed):
        ...
        self.rect = pygame.Rect(self.x, self.y, settings.tileSize,  settings.tileSize)
            ...
            if isinstance(layer, pytmx.TiledTileLayer):
                for x, y, gid, in layer:
                    ...
                    if not tileImage is None:
                        ...
                        self.image.blit(tileImage, (x * (self.tmxdata.tilewidth * self.scale), y * (self.tmxdata.tileheight * self.scale)))
        
        for tile_object in self.tmxdata.objects:
            if tile_object.name == 'player':
                self.pStartX, self.pStartY =  int(tile_object.x*self.scale), int(tile_object.y*self.scale)
...
class game:
    ...
    def new(self):
        ...
        charAnimation = {'u': [charImage], 'd': [charImage], 'l': [charImage], 'r': [charImage], 'fullArt': charImage}
        self.player = player(self.map.room.pStartX, self.map.room.pStartY, 'r', charAnimation, 1, 4, 10)
        gob1 = npc.goblin()
    ...
    def mainloop(self):
        ...
        while run:
            ...
            self.rendScreen([self.map.room, self.player] + self.map.room.sprites)
